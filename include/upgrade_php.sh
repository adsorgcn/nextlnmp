...
{
    Check_Stack
    if [[ "${Get_Stack}" = "nextlnmp" && "${Stack}" = "" ]]; then
        echo "Current Stack: ${Get_Stack}, please run: ./upgrade.sh php"
        exit 1
    elif [[ "${Get_Stack}" = "nextlnmpa" || "${Get_Stack}" = "lamp" ]] && [[ "${Stack}" = "nextlnmp" ]]; then
...
{
    Echo_nextLNMPA_Upgrade_PHP_Failed()
    {
        Echo_Red "======== upgrade php failed ======"
        Echo_Red "upgrade php log: /root/upgrade_a_php${Upgrade_Date}.log"
        echo "You upload upgrade_a_php.log to nextLNMP Forum for help."    
    }
    ...
    if [ "${Stack}" = "nextlnmp" ]; then
        if [[ -s /usr/local/php/sbin/php-fpm && -s /etc/init.d/php-fpm && -s /usr/local/php/etc/php.ini && -s /usr/local/php/bin/php ]]; then
            ...
            Echo_Red "======== upgrade php failed ======"
            Echo_Red "upgrade php log: /root/upgrade_nextlnmp_php${Upgrade_Date}.log"
            echo "You upload upgrade_nextlnmp_php.log to nextLNMP Forum for help."
        fi
    else
...
EOF
        ...
        if grep -q '^LoadModule mpm_event_module' /usr/local/apache/conf/httpd.conf && [ "${Stack}" != "nextlnmp" ]; then
